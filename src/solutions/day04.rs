...
impl Solution for Day04 {
    ...
    fn part_a(&self) -> String {
        let bingo = Bingo::parse_input(load("04"));
        let winning = bingo.solve();
        winning.0[winning.1].unchecked_sum(winning.2).to_string()
    }

    fn part_b(&self) -> String {
        let bingo = Bingo::parse_input(load("04"));
        let winning = bingo.loseing_solve();
        winning.0[winning.1].unchecked_sum(winning.2).to_string()
    }
}
...
impl Bingo {
    fn parse_input(raw_input: String) -> Self {
        ...
        let boards = Board::parse(raw_input);
...
impl Board {
    fn parse(raw_input: String) -> Vec<Self> {
        let inp = raw_input.replace('\r', "");
    ...
    fn unchecked_sum(&self, winning: u32) -> usize {
        let mut sum = 0;
